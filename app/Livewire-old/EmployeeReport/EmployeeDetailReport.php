...
class EmployeeDetailReport extends Component
{
    ...
    {
        return [
            'paymentMethod'    => ['organizationDetails'],
            'branch'           => ['aboutDetails'],
            'grade'            => ['aboutDetails'],
            'jobStatus'        => ['aboutDetails'],
            'department'       => ['aboutDetails'],
            'designation'      => ['aboutDetails'],
            'shift'            => ['aboutDetails'],
            'workMode'         => ['aboutDetails'],
            'role'             => [''],
            'reportingManager' => [''],
            'dealership'       => [''],
    ...
    public function generateReport()
    {
        ...
        $categoryFields = [
            ...
            'organizationDetails' => ['emp_policy_tax', 'leave_policy', 'emp_wo_policy', 'emp_attendance_policy', 'emp_sap_budget_code', 'emp_account_code', 'emp_cost_center', 'approver_manager_1', 'approver_manager_2', 'emp_bank_ifsc_code', 'emp_bank_name', 'emp_bank_branch_name', 'emp_bank_account_no', 'emp_bank_branch_code', 'emp_bank_micr_code', 'emp_bank_address_line1', 'emp_bank_address_line2', 'emp_salary_account_code', 'emp_salary_bank_ifsc_code', 'emp_salary_bank_name', 'emp_salary_bank_branch_name', 'emp_salary_bank_micr_code', 'emp_salary_bank_branch_code', 'emp_salary_bank_account_no', 'emp_profit_center', 'emp_region', 'emp_project', 'emp_paymentmode', 'emp_accountpurpose'],
        ...
        $filteredRecords = $records->map(function ($record) use ($categoryFields, $relationFields, $relationToFilter) {
                ...
                if ($isEnabled && isset($categoryFields[$category])) {
                    foreach ($categoryFields[$category] as $field) {
                        ...
                        if ($relationKey && isset($relationFields[$relationKey])) {
                            ...
                            $filteredData[$alias] = $record->$relationKey?->m_name ?? null;
                        } else {
            ...
            foreach ($relationFields as $relation => $fields) {
                ...
                if ($filterKey && !$this->filters[$filterKey]) continue;

                if ($relation === 'fh_employee_title' && $this->detailsFilter['aboutDetails']) continue;
    ...
    public function render()
    {
        ...
        return view('livewire.employee-report.employee-detail-report', compact(
            'employees', 'departments', 'shifts', 'designations', 'dealers', 'workMode',
            'attendanceStatuses', 'checkingMethods', 'branches', 'jobStatuses', 'grades',
            'paymentMethods', 'roles', 'reportingManagers', 'employeeStatus', 'employeeTypes'
        ));
    }
}