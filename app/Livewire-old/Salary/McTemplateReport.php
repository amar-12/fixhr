...
class McTemplateReport extends Component
{
    ...
    //         $this->dispatch('show-alert', ['type' => 'error', 'message' => 'No employees found for the selected payroll period.']);
    //         return;
    //     }
    ...
    //         'employee' => function ($q) {
    //             $q->select('emp_id', 'emp_full_name', 'emp_esic_no', 'emp_status');
    //         }
    //     ])
     ...
     public function generateReport()
    {
        ...
        return Excel::download(new class($businessName, $payroll, $empEsicData) implements WithMultipleSheets {
            protected $businessName, $payroll, $employees;
            public function __construct($businessName, $payroll, $employees)
            {
            ...
            public function sheets(): array
            {
                return [
                    // === SHEET 1: DATA ===
                    new class($this->businessName, $this->payroll, $this->employees) implements FromCollection, WithHeadings, WithEvents {
                        protected $businessName, $payroll, $employees;
                        public function __construct($businessName, $payroll, $employees)
                        {
                            $this->businessName = $businessName;
                        ...
                        public function collection()
                        {
                            return collect($this->employees)->map(function ($salary, $index) {
                                ...
                                return [
                                    ...
                                    'Total Monthly Wages' => $salary->ps_esic_monthly_gross ?? 0,
                                    'Reason Code for Zero workings days(numeric only: provide 0 for all other reasons- Click on the link for reference)' => '',
                                    'Last Working Day ( Format DD/MM/YYYY or DD-MM-YYYY)' => '',
                                ];
                            });
                    ...
                    new class implements \Maatwebsite\Excel\Concerns\FromArray, \Maatwebsite\Excel\Concerns\WithTitle, \Maatwebsite\Excel\Concerns\WithEvents {
                        public function array(): array
                        {
                            return [
                                ['Reason', 'Code', 'Note'],
