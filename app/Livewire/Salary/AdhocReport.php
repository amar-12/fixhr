...
class AdhocReport extends Component
{
    ...
    public function generateReport()
    {
        ...
        $transactions = $query
            ->get()
            ->sortBy(
                fn($item) => ($this->sortBy ?? 'emp_code') === 'emp_name'
                    ? strtolower($item->employee->emp_full_name ?? '')
                    : (int) ($item->employee->emp_code ?? 0)
            )
            ->values()
            ->toArray();
        ...
        $fileName = 'Adhoc_Report_' . now()->format('Y-m-d') . '.xlsx';
        $date = Carbon::now()->format('d-m-Y');
        return Excel::download(
            new AdhocPaymentsDeductions(
                $transactions,
                $this->businessId,
                $date,
    ...
    public function render()
    {
        ...
        $departments = Department::where('d_b_id', $this->businessId)
            ...
            ->limit(100)
            ->get();
        return view('livewire.salary.adhoc-report', compact(
            'financialYears',
            'payrollPeriods',
            'employees',
            'departments',
