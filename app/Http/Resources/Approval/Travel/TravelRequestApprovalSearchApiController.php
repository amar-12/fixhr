...
class TravelRequestApprovalSearchApiController extends JsonResource
{
    /**
    ...
    public function toArray(Request $request): array
    {
        ...
        // APPROVAL_STATUS => 156 - Processing, 157-Deduction, 192 - Recall
        $approval_status = MasterTable::where('m_group', 'APPROVAL_STATUS')
        ->whereNotIn('m_id', ['156','157','192'])
        ->get();
        ...
        // $log = $this->fh_plan_approval_log->first();
        $log = $this->fh_approval_log->first();
        ...
        $resourceData =  [
            ...
            'approval_status' => MasterTableResource::collection($approval_status)->all(),
            'fh_approval_log' => [
                'emp_code' => $log->fh_employee->emp_id ?? '',
                'emp_full_name' => $log->fh_employee->emp_fname ?? '',
            ],
