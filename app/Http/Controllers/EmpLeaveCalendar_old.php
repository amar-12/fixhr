...
class EmpLeaveCalendar extends Controller
{
	...
	{
		$leaveType = MasterTable::where('m_group', 'LIKE', '%LEAVE_TYPE%')->get();
		$leaveCat = MasterTable::where('m_group', 'LIKE', '%LEAVE_CATEGORY%')->get();
		$daySegment = MasterTable::where('m_group', 'LIKE', '%LEAVE_DAY_SEGMENT%')->get();
		return view('admin.setting.attendance-details.emp-leave-calendar', compact('leaveType', 'daySegment', 'leaveCat'));
	}
