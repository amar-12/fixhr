...
class ELPolicyController extends Controller
{
    public function index(Request $request)
    {
        $user = Auth::user();
        if ($user) {
            return view('admin.setting.attendance.el-policy');
        }
    }
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'ot_max_co_per_day'    => 'nullable|numeric|min:0|max:24',
            ...
            'max_ot_daily'         => 'required|numeric|min:0|max:12',
            'max_ot_monthly'       => 'required|numeric|min:0|max:300',
        ...
        try {
            ...
            $data = [
                ...
                'ot_max_co_per_day'      => $request->ot_max_co_per_day,
                'ot_shift_type'          => $request->shift_type,
                'ot_break_type'          => $request->break_type,
    ...
    public function overtime_index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $dynamicConditions = [
                ...
                [
                    ...
                    // request the relations with selected columns (if your relation keys differ, adjust below)
                    'relation' => [
            ...
            // total records for this business & date
            ...
            foreach ($list as $val) {
                ...
                // dd($val->ot_id, $val->ot_requested_status, $val->ot_am_id, $user->emp_id);
                if ($approvalData) {
                ...
                // Resolve employee object: helper may have returned relation as fh_employee or employee
        ...
        if (!$request->ajax()) {
            ...
            $relevantMSPRequests = OtApprovalStatus::where('ot_b_id', $user->emp_b_id)
