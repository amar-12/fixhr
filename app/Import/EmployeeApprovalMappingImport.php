...
class EmployeeApprovalMappingImport implements ToCollection
{
    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $index => $row) {
            ...
            // Skip rows with no valid approver data (silently, no error)
            if (empty($approvers)) {
                continue;
            }
    ...
    private function hasPendingApprovals($businessId, $employeeId, $moduleId)
    {
        ...
        $modules = [
            ...
            // 562 Comp Off
            562 => [
                'model' => \App\Models\CompOff::class,
                'business_column' => 'co_b_id',
                'stage_column' => 'co_stage_completed',
                'employee_column' => 'co_emp_id',
            ],
        ];
