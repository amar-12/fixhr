...
class DesignationImport implements ToCollection
{
    ...
    public function collection(Collection $rows)
    {
        ...
        foreach ($rows as $key => $row) {
            ...
            $designation_name    = trim($row[1]) ?? null;

            $rowErrors = [];
            if (!$designation_name) {
                $rowErrors[] = "Designation Name '{$designation_name}' not found.";
            }

            // Unique check
            if (
                !empty($designation_name) &&
                Designation::where('dg_name', $designation_name)
                      ->where('dg_b_id', $this->user->emp_b_id)
                      ->exists()
            ) {
                $rowErrors[] = "Designation Name '{$designation_name}' already exists.";
            }
            ...
            try {
                Designation::create([
                    'dg_b_id' => $this->user->emp_b_id,
                    'dg_name' => $designation_name
                ]);
