...
class MonthlyInOutReport implements FromArray, WithHeadings, WithEvents
{
    ...
    public function array(): array
    {
        foreach ($this->exportData as &$row) {
            if (isset($row['dailyAttendance'])) {
                $dailyAttendance = $row['dailyAttendance'];

                for ($day = 1; $day <= $this->daysInMonth; $day++) {
                    $checkIn = isset($dailyAttendance[$day - 1]['checkInTime']) ? $dailyAttendance[$day - 1]['checkInTime'] : '';
                    $checkOut = isset($dailyAttendance[$day - 1]['checkOutTime']) ? $dailyAttendance[$day - 1]['checkOutTime'] : '';
                    $row["Day {$day}"] = $checkIn . "\n" . $checkOut;
                }

                // Optionally remove the original dailyAttendance if not needed
                unset($row['dailyAttendance']);
            }
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                // Align header row (Row 5 after insert)
                $headerRow = 5;
                ...
                // Apply wrap text and vertical alignment for day columns
                $dataStartRow = $headerRow + 1;
                $dataEndRow = $worksheet->getHighestRow();

                $startColIndex = 3; // Column C = index 3
                $endColIndex = $startColIndex + $this->daysInMonth - 1;

                for ($i = $startColIndex; $i <= $endColIndex; $i++) {
                    $column = \PhpOffice\PhpSpreadsheet\Cell\Coordinate::stringFromColumnIndex($i);
                    $sheet->getStyle("{$column}{$dataStartRow}:{$column}{$dataEndRow}")->applyFromArray([
                        'alignment' => [
                            'wrapText' => true,
