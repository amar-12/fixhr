...
class TADAPaymentSheet implements FromCollection, WithStyles
{
    protected $data;
    protected $user;
    ...
    */
    public function collection()
    {
        ...
        $exportData[] = [' '];
        $exportData[] = [' ', 'TADA Payment Sheet'];
        $exportData[] = [' '];
        $exportData[] = [' ','TADA Payment Sheet As On Date', date('d-M-Y')];
        ...
        foreach ($this->data as $item) {
            ...
            $rowData = [
                'S. No.' => $i++,
                // 'C' => 'N',
                'Trp Id' => $item->trp_unique_id ?? '',
                'TRP From Date' => $item->trp_start_date ?? '',
                'TRP End Date' => $item->trp_end_date ?? '',
                'Trp Name' => $item->trp_name ?? '',
                'Trp Remarks' => $item->trp_remarks ?? '',
                'Beneficiary Code' => '',
                'Beneficiary Account Number' => isset($item->fh_employee) ? $item->fh_employee->emp_bank_account_no : '',
                // 'Instrument Amount' => isset($item->fh_tada_claim) ? $item->fh_tada_claim->tc_payed_amount : '',
                'Amount' => isset($item->fh_tada_claim) ? $item->fh_tada_claim->tc_payed_amount : '',
                'Beneficiary Name' => '',
                'Drawee Location' => '',
                'Print Location' => '',
                'Bene Address 1' => '',
                ...
                'Bene Address 5' => '',
                'Instruction Reference Number' => '',
                'Customer Reference Number' => isset($item->fh_employee) ? 'TADA - ' .$item->fh_employee->emp_full_name : '',
                ...
                'Payment details 7' => '',
                'Cheque Number' => '',
                // 'Chq / Trn Date' => '27-08-2014',
                'Claim Id' => isset($item->fh_tada_claim) ? $item->fh_tada_claim->tc_unique_id : '',
                'Claim Date' => isset($item->fh_tada_claim) && $item->fh_tada_claim->created_at ? $item->fh_tada_claim->created_at->format('d-m-Y') : '',
            ...
            $item->update(['trp_tada_payment_processed'=>1]);
            $exportData[] = $rowData;
        }
