...
<script>
    $(document).ready(function() {
        var table = $('#device-verification-table').DataTable({
            processing: false, // Disable default DataTables processing overlay
            serverSide: true,
            ajax: {
                url: "{{ route('admin.requests.device_verification.datatable') }}",
                data: function(d) {
        ...
        table.on('draw', function() {
            $('#device-verification-table tbody tr').each(function() {
       ...
       /*         var row = $checkbox.closest('tr');
                var statusCell = row.find('td:eq(4)'); // Status column
                var status = statusCell.text().trim();
                
                // Only check pending devices
                if (status === 'Pending') {
                    $checkbox.data('programmatic', true).prop('checked', checked);
                    $checkbox.removeData('programmatic');
                } else {
                    $checkbox.data('programmatic', true).prop('checked', false);
                    $checkbox.removeData('programmatic');
                } */
            });
        ...
        /*    var $checkbox = $(this);
            var row = $checkbox.closest('tr');
            var statusCell = row.find('td:eq(4)'); // Status column (5th column, 0-indexed)
            var status = statusCell.text().trim();
            
            // Check if device is pending
            if (status !== 'Pending') {
                $checkbox.prop('checked', false);
                // Only show alert if this is a user action (not programmatic)
                if (!$(this).data('programmatic')) {
                    Swal.fire({
                        icon: 'warning',
                        title: 'Selection Restricted',
                        text: 'Only pending devices can be selected for bulk actions.',
        ...
        $('#verify-selected-btn').on('click', function() {
            ...
            $.ajax({
                url: "{{ route('admin.requests.device_verification.bulk_verify') }}",
                method: 'POST',
                data: {
                    _token: '{{ csrf_token() }}',
                    ids: ids
                ...
                success: function(res) {
                    ...
                    Swal.fire({
                        icon: 'success',
                        title: 'Success!',
                        text: res.message || 'Selected devices verified successfully.',
                        showConfirmButton: false,
                        timer: 2000,
                        timerProgressBar: true,
                        toast: true,
                ...
                error: function(xhr) {
                    var errorMessage = 'Bulk verify failed.';
        ...
        // Reject Selected button click
        $('#reject-selected-btn').on('click', function() {
            Swal.fire({
                title: 'Are you sure?',
                text: 'Are you sure you want to reject the selected devices? This action cannot be undone.',
                icon: 'warning',
                ...
                if (result.isConfirmed) {
                    ...
                    // Show loading state
                    var $btn = $('#reject-selected-btn');
                    var originalText = $btn.text();
                    $btn.prop('disabled', true).text('Rejecting...');
                    
                    $.ajax({
                        url: "{{ route('admin.requests.device_verification.bulk_reject') }}",
                        method: 'POST',
                        data: {
                            _token: '{{ csrf_token() }}',
                            ids: ids
                        ...
                        success: function(res) {
                            ...
                            Swal.fire({
                                icon: 'success',
                                title: 'Rejected!',
                                text: res.message || 'Selected devices rejected successfully.',
                                showConfirmButton: false,
                                timer: 2000,
                                timerProgressBar: true,
                                toast: true,
                        ...
                        error: function(xhr) {
                            var errorMessage = 'Bulk reject failed.';
                            if (xhr.responseJSON && xhr.responseJSON.message) {
                                errorMessage = xhr.responseJSON.message;
                            }
                            Swal.fire({
...
<div>
    <div class="p-0 pb-4">
        <ol class="breadcrumb breadcrumb-arrow m-0 p-0" style="background: none;">
            ...
            <li class="active"><span><b>Device Verification</b></span></li>
        </ol>
    </div>
    <div class="row">
        <div class="col-xl-12 col-md-12 col-lg-12">
            <div class="card card-custom">
                <div class="card-header border-0">
                    <h4 class="card-title">Device Verification</h4>
                </div>
                <div class="card-body">
                    <div class="row g-3">
                        ...
                        <div class="row">
                            ...
                            <div class="col-lg-2 col-md-4 col-sm-6 col-12">
                                ...
                                <select id="daily_branchFilter" data-filter class="form-select search_test filter_border"    style="border-radius: 20px;">
                            ...
                            <div class="col-lg-2 col-md-4 col-sm-6 col-12">
                                ...
                                <select id="daily_departmentFilter" data-filter
                            ...
                            <div class="col-lg-2 col-md-4 col-sm-6 col-12">
                                ...
                                <select id="daily_designationFilter" data-filter
                                    class="form-select search_test filter_border"   style="border-radius: 20px;">
                                    <option value="">All</option>
                            ...
                            <div class="col-lg-2 col-md-4 col-sm-6 col-12">
                                ...
                                <select id="daily_activeFilter" data-filter class="form-select search_test filter_border"   style="border-radius: 20px;">
                                    <option value="">All</option>
                            ...
                            <div class="col-lg-2 col-md-4 col-sm-6 col-12">
                                ...
                                <input type="date" id="fromDate" value="" class="form-control filter_border"
                    ...
                    <div class="table-responsive">
                        <table class="table display table-hover table-vcenter text-wrap border-bottom" id="device-verification-table">
                            <thead>
                                <tr>
                                    <th>S No.</th>
                                    <th>Employee Code</th>
