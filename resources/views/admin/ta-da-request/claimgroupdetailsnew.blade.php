...
@section('content')
    @php
        ...
        $rawTravelDetailSumAmt =  $rawTravelTypeLocal ?  $rawTravelDetails->sum('trd_net_amount') : App\Models\TadaRequestDetail::whereHas('fh_policy_tada_travel_vehicle', function($query) {
                        ...
                        <table class="table text-nowrap">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="py-1">:</td>
                                    <td class="py-1"> <span class="badge {{ $claimData->fh_employee->emp_status == 71 ? 'badge-success-light' : 'badge-warning-light' }}">{{$claimData->fh_employee->emp_status == 71 ? 'Active' : 'Inactive'}}</span> </td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                        ...
                        <table class="table text-nowrap">
                            @php
                                $claimDetails = [
                                    ...
                                    'Travel Start'=>  \Carbon\Carbon::parse($claimData->fh_tada_request_plan->trp_start_date.' '.$claimData->fh_tada_request_plan->trp_start_time)->format('d-M-Y h:i A')  ?? 'N/A',
                                    'Travel End'=>  \Carbon\Carbon::parse($claimData->fh_tada_request_plan->trp_end_date.' '.$claimData->fh_tada_request_plan->trp_end_time)->format('d-M-Y h:i A')  ?? 'N/A',
                                ];
                                            ...
                                            <div class="tab-pane active" id="tabSelfExpSummary">
                                                                    ...
                                                                    <table class="table table-bordered table-hover text-nowrap">
                                                                        <tr class=" ">
                                                                            ...
                                                                            @if (($approvalData || $canApprove) && !$displayDeductionHandler)
                                                                                <th class="text" style="width: 1%">Deduction</th> <!-- Added new column for Deduction -->
                                                                            @endif
                                                                        </tr>
                                                                        ...
                                                                        @foreach ($claimData->fh_tada_request_plan->fh_tada_expenses->where('te_paid_by', 'self')->groupBy('fh_expense_type.m_name') as $expenseType => $expenses)
                                                                          ...
                                                                          @endphp
                                                                            <tr>
                                                                                ...
                                                                                <td class="tdExpenseType">{{ $expenseType }}</td>
                                                                                <td>₹ {{ round($expenses->sum('te_amount') + $expenses->sum('te_taxes')) }}</td>
                                                                                <td>₹ {{ round($expenses->sum('te_deviation')) }}</td>
                                                                                <td>₹ {{ round($expenses->sum('te_amount') + $expenses->sum('te_taxes')) - round($expenses->sum('te_deviation')) }}</td>
                                                                                @if (($approvalData || $canApprove) && !$displayDeductionHandler)
                                                                                    <td>
                                                                                        <!-- Added input field for deductions -->
                                                                                        <input type="number" name="deduction[{{ $exp_type_id }}]" id="{{$exp_type_id.'_deduction'}}" data-payableAmount="{{ round($expenses->sum('te_amount') + $expenses->sum('te_taxes')) - round($expenses->sum('te_deviation')) }}" class="form-control deduction-input" step="0.01" min="0" placeholder="₹ 0">
                                                                                    </td>
                                                                                @endif
                                                                            </tr>
                                                                        @endforeach
                                                                        ...
                                                                        @if ($rawPlanData->trp_is_details_added)
                                                                            <tr>
                                                                                ...
                                                                                <td><!-- Optional deduction input for TA --></td>
                                                                            </tr>
                                                                        @endif
                                                                        ...
                                                                        <tr>
                                                                            ...
                                                                            <td><!-- Optional deduction input for DA --></td>
                                                                        </tr>

                                                                        <tr>
                                                                            <td colspan="4" class="font-weight-semibold text-end">Subtotal<span> Total Expense Amount (Inc. Taxes), Travel Allowance, and DA) after Deduction of Deviation.</td>
                                                ...
                                                @if ($actionRoute !== 'claim-request.request.show' && $actionRoute !== 'claim-request-is-paid.request.show')
                                                    ...
                                                    @if ($approvalData && !$displayDeductionHandler)
                                                        ...
                                                        <x-approval-form :moduleName="$claimData?->fh_module?->m_name" :masterApproveBtn="$masterApproveBtn" :primaryId="$claimData->tc_id" :moduleId="$claimData->tc_module_id" actionUrl="{{route('approve.claim')}}" />
                                                    @endif
                                                @endif
                                                            ...
                                                            @if ($travelDetailsItem->trd_segments != null)
                                                                <div class="card-body my-5 outer-border position-relative pb-0">
                                                                    ...
                                                                    <div class="row mt-2">
                                                                        <div class="col-md-7">
                                                                            <div class="row">
                                                                                ...
                                                                                <div class="col-md-12">
                                                                                    <p><b>Tap Location Longitute Latituted</b></p>
                                                                                    <ul class="timeline"
                                                                                                ...
                                                                                                <li
                                                                                                    ...
                                                                                                    <span
                                                                                                        ...
                                                                                                        <span
                                                                                                            ...
                                                                                                            &nbsp;
                                                                                                            ( {{ $location->location ?? 'N/A' }})</span></span>
                                                                                                </li>
                                                                                            </ul>
                                                                                        @endforeach
                                                                                    </ul>
                                                                    ...
                                                                    <ul class="text-start row">
                                                                        @foreach ($documents as $dkey => $document)
                                                                            @php
                                                                                $position = strpos($document, 'travelDetailsItem_');
                                                                                if ($position !== false) {
                                                                                    $displayName = substr($document, $position);
                                                                                } else {
                                                                                    // If "PlanDetail_" is not found, use the full file name
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (data.status == true) {
                        ...
                        var baseUrl = $('#ajaxCall').val();
                        window.location.href = baseUrl + "/admin/ta-da-request/claim-request/show/" + tcId;
                    } else {
                        Swal.fire({
                            icon: "warning",
                            text: data.message,
