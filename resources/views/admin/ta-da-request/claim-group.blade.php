...
$statusFilter = CommonUtils::getCustomModelData(
    new MasterTable(),
    [
        ['method' => 'where', 'args' => ['m_group', 'APPROVAL_STATUS']],
        ['method' => 'whereIn', 'args' => ['m_id', [157, 451, 140, 172, 141, 412, 174, 170, 452]]],
    ]
);
        ...
        <div class="col-xl-12 col-md-12 col-lg-12">
            <div class="card">
                <div class="card-header border-0">
                    <h4 class="card-title">Claim Group Request  </h4>
                </div>
                <div class="card-body">
                    <div class="row">
                        <div class="col-sm-1">
                        ...
                        <div class="col-sm-2" style="margin-top: 10px;">
                            @if ($actionRoute == 'claim.request.index1')
                ...
                <div class="modal-body">
                    <div class="table-responsive">
                        <table class="table table-bordered">
                            ...
                            <tr>
                                <th colspan="5">Subtotal Total Expense Amount (Inc. Taxes, Travel Allowance, and DA) after Deduction of Deviation.</th>
                                <td id="subtotal"></td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                <th colspan="5">NET PAYABLE AMOUNT</th>
                                <td id="netPayable"></td>
                            </tr>
                            <tr>
                                <th colspan="5">Approval Or Reject</th>
    ...
    <script>
        $(document).ready(function() {
            ...
            $(document).on('click', '.open-claim-modal', function() {
                ...
                $.ajax({
                    url: '{{ route("travel.claim-group.details", ":id") }}'.replace(':id', tcId),
                    type: 'POST',
                    data: {
                        tc_id: tcId,
                        _token: '{{ csrf_token() }}'
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            $('#claimModal .modal-body').empty();

                            // Create a container for all claims
                            var claimsContainer = $('<div class="claims-container"></div>');

                            // Add employee information
                            var employeeInfo = `
                                <div class="employee-info mb-4">
                                    <h5>Employee: ${response.additional_details.employee_name}</h5>
                                    <p>Status: <span class="badge badge-${response.additional_details.status === 'Approved' ? 'success' :
                                      response.additional_details.status === 'Rejected' ? 'danger' : 'warning'}">
                                        ${response.additional_details.status}
                                    </span></p>
                                </div>
                            `;
                            claimsContainer.append(employeeInfo);

                            // Process each claim
                            $.each(response.claims, function(claimIndex, claim) {
                                var claimTable = `
                                    <div class="single-claim mb-5">
                                        <h6 class="claim-header">Claim ID: ${response.claim_id || response.additional_details.claim_id}</h6>
                                        <div class="table-responsive">
                                            <table class="table table-bordered">
                                                <thead>
                                                    <tr>
                                                ...
                                                <tbody class="expensesTableBody">
                                `;

                                // Add expenses rows
                                if (claim.expenses && claim.expenses.length > 0 && claim.expenses[0].name !== "N/A") {
                                    $.each(claim.expenses, function(expenseIndex, expense) {
                                        claimTable += `
                                            <tr>
                                                <td>${expenseIndex + 1}</td>
                                                <td>${expense.name}</td>
                                                <td>₹ ${expense.amount.toLocaleString()}</td>
                                                <td>₹ ${expense.deviation.toLocaleString()}</td>
                                                <td>₹ ${expense.payable_amount.toLocaleString()}</td>
                                                <td>₹ ${expense.deduction.toLocaleString()}</td>
                                            </tr>
                                        `;
                                    });
                                } else {
                                    // Default row if no valid expenses
                                    claimTable += `
                                        <tr>
                                            <td>1</td>
                                            <td>Meal</td>
                                ...
                                // Add DA row if exists
                                if (claim.da && claim.da > 0) {
                                    claimTable += `
                                        <tr>
                                            <td></td>
                                            <td>DA</td>
                                            <td>₹ ${claim.da.toLocaleString()}</td>
                                            <td></td>
                                            <td></td>
                                            <td></td>
                                        </tr>
                                ...
                                // Calculate subtotal
                                var subtotal = (parseFloat(claim.amount) || 0) + (parseFloat(claim.da) || 0);
                                ...
                                claimTable += `
                                                ...
                                                <tfoot>
                                                    <tr>
                                                        <th colspan="5">Subtotal Total Expense Amount</th>
                                                        <td>₹ ${subtotal.toLocaleString()}</td>
                                                    </tr>
                                                    <tr>
                                                        <th colspan="5">Advance</th>
                                                        <td>₹ ${(claim.advance || 0).toLocaleString()}</td>
                                                    </tr>
                                                    <tr>
                                                        <th colspan="5">Deduction</th>
                                                        <td>₹ ${(claim.deduction || 0).toLocaleString()}</td>
                                                    </tr>
                                                    <tr>
                                                        <th colspan="5">NET PAYABLE AMOUNT</th>
                                                        <td>₹ ${(claim.net_payable || subtotal).toLocaleString()}</td>
                                                    </tr>
                                                </tfoot>
                                            </table>
                                        </div>
                                ...
                                claimsContainer.append(claimTable);
                            });

                            // Add the container to modal body
                            $('#claimModal .modal-body').append(claimsContainer);
                            ...
                            Swal.fire({
                                icon: 'error',
                                text: 'Failed to fetch claim details: ' + (response.message || 'Unknown error'),
                                timer: 3000,
                    ...
                    error: function(xhr, status, error) {
                        Swal.fire({
                            icon: 'error',
                            text: xhr.status === 404 ? 'Claim not found' : 'An error occurred: ' + error,
                            timer: 3000,
                            showConfirmButton: false
                        });
                    }
