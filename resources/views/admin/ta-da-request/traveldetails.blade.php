                ...
                <section id="reverbDynamicSection" class="p-0 m-0">
                    ...
                    @if (count($data->fh_plan_approval_log) > 0)
                        ...
                        <div class="card approval-details" id="claim_approval_details" style="display: none;">
                          ...
                          <div class="card approval-details" id="advance_approval_details" style="display: none;">
                            <div class="card-header px-3">
                                <div class="card-title">Advance Approval Details</div>
                            </div>
                            <div class="card-body p-0">
                                <div class="table-responsive">
                                    <table class="table table-vcenter text-nowrap border-bottom">
                                            ...
                                            <tbody>
                                                <tr>
                                                    <td colspan="7" class="text-center text-muted py-5">
                                                        <div
                                                            class="d-flex flex-column align-items-center justify-content-center">
                                                            <span class="fw-semibold fs-5 text-secondary">Advance
                                                                    ...
                                                                    <div class="teams">
                                                                        <div class="d-flex align-items-center text-muted">
                                                                            @php
                                                                                ...
                                                                                $variation_distance = $item->trd_total_tolerance ?? 0;
                                                                                $eligibility = $item->fh_policy_tada_travel_allowance->pttv_eligibility ?? 0;
                                                                                $total_distance = $travel_distance + $variation_distance;
                                                                                $total_amount = $eligibility > 0 ? ($total_distance * $eligibility) : 0;
                                                                            @endphp
                                                                            <h6 class="mt-1">
                                                                                Amount<small> ({{ $eligibility > 0 ? '₹' . number_format($eligibility, 2) . ' /km' : 'N/A' }}) </small>:
                                                                            </h6>
                                                                            <div class="ms-1">
                                                                                ₹ {{ $eligibility > 0 ? number_format($total_amount, 2) : 'N/A' }}
                                                                            </div>
                                                                        </div>
                                                                        <div class="d-flex align-items-center text-muted">
                                                                            <h6 class="mt-1">Distance :</h6>
                                                                            <div class="ms-1 text-muted">
                                                                                {{ $travel_distance > 0 ? number_format($travel_distance, 2) . ' KM' : 'N/A' }}
                                                                            </div>
                                                                        </div>
                                                                        <div class="d-flex align-items-center text-muted">
                                                                            <h6 class="mt-1">Variation :</h6>
                                                                            <div class="ms-1 text-muted">
                                                                                {{ $variation_distance > 0 ? number_format($variation_distance, 2) . ' KM' : 'N/A' }}
                                                                            </div>
                                                                        </div>
                                                                        <div class="d-flex align-items-center text-muted">
                                                                            <h6 class="mt-1">Total Distance :</h6>
                                                                            <div class="ms-1 text-muted">
                                                                                {{ $total_distance > 0 ? number_format($total_distance, 2) . ' KM' : 'N/A' }}
                                                                            </div>
                                                                        </div>
                                                                    </div>
                                                                    <ul class="timeline">
                                                                        <li class="primary" style="margin: 5px 0;">
                                                                            <span
                                                                                class="font-weight-semibold fs-16 ms-3">Source</span>
                                                                            <p class="mb-1 pb-0 text-muted fs-14 ms-3 mt-1">
                                                                                {{ $item->trd_source ?? 'N/A' }}
                                                                            </p>
                                                                        </li>
                                                                        <li class="success mt-6">
                                                                            ...
                                                                            <p
                                                                                class="mb-1 pb-0 text-muted fs-14 ms-3 mt-1">
        ...
        <div class="modal-dialog modal-xl modal-dialog-centered" role="document" style="max-width: 1200px;">
            <div class="modal-content">
                <div class="modal-header d-flex justify-content-between align-items-center">
                    <h5 class="modal-title" style="font-size:20px;" id="largemodalLabel">
                        Employee Travel Path  - 
                        <small class="text-muted" style="font-size:16px;">
                            ...
                            $total_distance = $travel_distance + $variation_distance;
                             @endphp
                                Travel Start :  {{ \Carbon\Carbon::parse($data->trp_start_date . ' ' . $data->trp_start_time)->format('d-M-Y h:i A') ?? 'N/A' }} | Travel End : {{ \Carbon\Carbon::parse($data->trp_end_date . ' ' . $data->trp_end_time)->format('d-M-Y h:i A') ?? 'N/A' }} |
                                Total Distance : {{ $total_distance > 0 ? number_format($total_distance, 2).' KM' : 'N/A' }}
                ...
                <div class="modal-body">
                    ...
                    <div id="travelPathContainer">
                        <h5>Employee Travel History - {{ $total_distance > 0 ? number_format($total_distance, 2).' KM' : 'N/A' }}</h5>
...
<script>
    ...
    <script>
        ...
        function get_advance() {
            ...
            // Check if advance data exists
            var hasAdvanceData = {{ count($data->fh_tada_advance_approval_log) > 0 ? 'true' : 'false' }};
            
            if (hasAdvanceData) {
                // Show modal if advance data exists
                var modal = new bootstrap.Modal(document.getElementById('advanceApprovalModal'));
                modal.show();
            } else {
                // Show card if no advance data
                $("#advance_approval_details").toggle();
            }
        }
    </script>
    <!-- <script>
