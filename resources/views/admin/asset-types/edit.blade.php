    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                <h5 class="mb-0">Edit Asset Type: {{ $assetType->name }}</h5>
            </div>
            <div class="card-body">
                <form method="POST" action="{{ route('asset-types.update', $assetType) }}" id="assetTypeForm">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="col-md-6">
                            <div class="mb-3">
                                ...
                                <input type="text" name="description" id="description" 
                                       ...
                                       value="{{ old('description', $assetType->description) }}" placeholder="Optional description">
                                @error('description')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <div class="form-check p-3 border rounded">
                                    <input class="form-check-input" type="radio" name="structure_type" id="simple" value="simple" 
                                           {{ old('structure_type', $assetType->structure_type) === 'simple' ? 'checked' : '' }}>
                            ...
                            <div class="col-md-6">
                                <div class="form-check p-3 border rounded">
                                    <input class="form-check-input" type="radio" name="structure_type" id="component_based" value="component_based"
                                           {{ old('structure_type', $assetType->structure_type) === 'component_based' ? 'checked' : '' }}>
                    ...
                    <!-- Simple Asset Fields -->
                    <div id="simple-fields" class="structure-section" style="{{ $assetType->structure_type === 'simple' ? '' : 'display: none;' }}">
                        ...
                        <div id="simple-fields-container">
                            @if($assetType->structure_type === 'simple')
                                @foreach($assetType->fields as $index => $field)
                                <div class="field-row mb-4 p-3 border rounded" data-field-id="{{ $field->id }}">
                                    <div class="d-flex justify-content-between align-items-center mb-3">
                                        ...
                                        <div class="col-md-3">
                                            <div class="mb-3">
                                                ...
                                                <input type="text" name="fields[{{ $index }}][attributes][placeholder]" class="form-control" 
                                                       value="{{ $field->attributes['placeholder'] ?? '' }}" placeholder="Enter placeholder text">
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="conditional-logic-section mt-3">
                                        <div class="form-check mb-3">
                                            <input type="checkbox" class="form-check-input enable-condition" 
                                                   id="enable_condition_{{ $index }}" {{ $field->hasCondition() ? 'checked' : '' }}>
                                            <label class="form-check-label" for="enable_condition_{{ $index }}">
                                        ...
                                        <div class="condition-settings" style="{{ $field->hasCondition() ? '' : 'display: none;' }}">
                                            <div class="row">
                                                <div class="col-md-4">
                                                    <div class="mb-3">
                                                ...
                                                <div class="col-md-4">
                                                    <div class="mb-3">
                                                        ...
                                                        <select name="fields[{{ $index }}][condition_operator]" class="form-select">
                                                            <option value="==" {{ $field->condition_operator === '==' ? 'selected' : '' }}>equals</option>
                                                            <option value="!=" {{ $field->condition_operator === '!=' ? 'selected' : '' }}>not equals</option>
                                                            <option value="contains" {{ $field->condition_operator === 'contains' ? 'selected' : '' }}>contains</option>
                                                            <option value="not_contains" {{ $field->condition_operator === 'not_contains' ? 'selected' : '' }}>does not contain</option>
                                                        </select>
                                                    </div>
                                                </div>
                                                <div class="col-md-4">
                                                    <div class="mb-3">
                                                        ...
                                                        <input type="text" name="fields[{{ $index }}][condition_value]" class="form-control" 
                                                               value="{{ $field->condition_value }}" placeholder="Value to match">
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                    ...
                    <div id="component-fields" class="structure-section" style="{{ $assetType->structure_type === 'component_based' ? '' : 'display: none;' }}">
                        <div class="d-flex justify-content-between align-items-center mb-3">
                            ...
                            <div class="col-md-4 mb-3">
                                <div class="form-check p-3 border rounded">
                                    <input class="form-check-input" type="checkbox" name="components[]" 
                                           ...
                                           {{ $assetType->components->contains($component->id) ? 'checked' : '' }}>
...
$(document).ready(function() {
    let fieldIndex = {{ $assetType->fields->count() }};
    ...
    $(document).on('click', '.add-option', function() {
        ...
        const currentFieldIndex = fieldIndexMatch ? fieldIndexMatch[1] : fieldIndex;
    ...
    // Initialize conditional field options on page load
    updateConditionalFieldOptions();
    
    // Set existing condition values
    @if($assetType->structure_type === 'simple')
        @foreach($assetType->fields as $index => $field)
            @if($field->hasCondition())
                setTimeout(function() {
                    $('select[name="fields[{{ $index }}][condition_field_slug]"]').val('{{ $field->condition_field_slug }}');
                }, 100);
            @endif
        @endforeach
    @endif
