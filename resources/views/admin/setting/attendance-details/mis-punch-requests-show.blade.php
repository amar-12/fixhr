...
@section('title', 'Mis-Punch Details')
            ...
            <div class="py-0 bd-highlight">
                <div>
                    <ol class="breadcrumb breadcrumb-arrow m-0 p-0" style="background: none;">
                        ...
                        <li><a href="/admin/requests/mis-punch">Mis-Punch</a></li>
                        <li class="active"><span><b>Mis-Punch Details</b></span></li>
                    </ol>
                </div>
            </div>
        </div>
                ...
                <div class="col-xl-12 col-md-12">
                    <div class="card border overflow-hidden">
                        <div class="card-header p-3">
                            <div>
                                <h4 class="card-title">Mis-Punch Request</h4>
                            </div>
                        </div>
                                ...
                                @php
                                    $details = [
                                        'Applied Date' => $data->created_at
                                            ...
                                            : 'N/A',
                                        'Date' => $data->ae_date ? $data->ae_date->format('d-m-Y') : 'N/A',
                                        'Time Type' => optional($data->fh_mispunch_type)->m_name ?? 'N/A',
                                        'Pre. Check In' => isset($records->atd_check_in_time) ? (new DateTime($records->atd_check_in_time))->format('H:i') : 'N/A',
                                        'Pre. Check Out' => isset($records->atd_check_out_time) ? (new DateTime($records->atd_check_out_time))->format('H:i') : 'N/A',
                                        'Check In' => isset($data->ae_in_time) ? \Carbon\Carbon::parse($data->ae_in_time)->format('H:i') : 'N/A',
                                        'Check Out' => isset($data->ae_out_time) ? \Carbon\Carbon::parse($data->ae_out_time)->format('H:i') : 'N/A',
                                        'Working Hour' => ($data->ae_in_time && $data->ae_out_time)
                                            ? \Carbon\Carbon::parse($data->ae_out_time)->diff(\Carbon\Carbon::parse($data->ae_in_time))->format('%H.%I')
                                            : 'N/A',


                                        'Reason' => optional($data->fh_mispunch_reason)->m_name ?? 'N/A',
                        ...
                        @if ($approvalData)
                        <div class="card-header p-3">
                            <h3 class="card-title">Mis-punch Approval Or Reject</h3>
                        </div>
                        <div class="card-body">
                    ...
                    @elseif ($canApprove)
                        <x-approval-form :moduleName="$data?->fh_module?->m_name" :masterApproveBtn="$masterApproveBtn" :primaryId="$data->ae_id" :moduleId="$data->ae_module_id" actionUrl="{{ route('approve.mis-punch') }}" />
                    @endif
                    </div>
                    ...
                    <div class="card mt-3">
                        <div class="card-body">
                            <ul class="timeline">
                                @foreach ($mspApprovalLogs as $item)
                                    @php
