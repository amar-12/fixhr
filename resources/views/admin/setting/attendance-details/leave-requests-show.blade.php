            ...
            <div class="py-0 bd-highlight">
                <div>
                    <ol class="breadcrumb breadcrumb-arrow m-0 p-0" style="background: none;">
                        ...
                        <li><a href="/admin/requests/leave">Leave</a></li>
                        <li class="active"><span><b>Leave Details</b></span></li>
                    </ol>
                </div>
            </div>
        </div>
                            ...
                            <table class="table text-nowrap">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="py-1">:</td>
                                        <td class="py-1">
                                            <span class="badge {{ $data->fh_employee && $data->fh_employee->emp_status == 71 ? 'badge-success-light' : 'badge-warning-light' }}">
            ...
            <div class="row">
                <div class="col-xl-12 col-md-12">
                    <div class="card border overflow-hidden">
                            ...
                            <div class="row">
                                ...
                                $details = [
                                    'Applied Date' => $data->created_at
                                        ...
                                        : 'N/A',
                                    'Leave Type' => optional($data->fh_leave_day_type)->m_name ?? 'N/A',
                                    'Day Segments' => optional($data->fh_leave_day_segment)->m_name ?? 'N/A',
                                    'From Date' => !empty($startDate)
                                        ? Carbon::parse($startDate)->format('d-M-Y')
                                        : ($data->lvr_start_date
                                            ? Carbon::parse($data->lvr_start_date)->format('d-M-Y')
                                            : 'N/A'),
                                    'To Date' => !empty($endDate)
                                        ? Carbon::parse($endDate)->format('d-M-Y')
                                        : ($data->lvr_end_date
                                            ? Carbon::parse($data->lvr_end_date)->format('d-M-Y')
                                            : 'N/A'),
                                    'Reason' => $data->lvr_reason ?? 'N/A',
                                    'Status' => optional($data->fh_approval_status)->m_name ?? 'N/A',
                                ];

                                    // Adding category details dynamically with correct format
                                    if ($categoryWiseLeave->isNotEmpty()) {
                                        foreach ($categoryWiseLeave as $category) {
                                            $details[$category['name']] = $category['count'];
                                        }
                                    }
                                @endphp
                                ...
                                @php
                                    $value =
                                        json_decode($data->lvr_documents) ?? 'N/A';
                                @endphp
                        ...
                        @if ($approvalData)
                            <div class="card-header p-3">
                                <h3 class="card-title">Leave Approval Or Reject</h3>
                            </div>
                            <div class="card-body">
                        ...
                        @elseif ($canApprove)
                            <x-approval-form :moduleName="$data?->fh_module?->m_name" :masterApproveBtn="$masterApproveBtn" :primaryId="$data->lvr_id" :moduleId="$data->lvr_module_id" actionUrl="{{route('approve.leave')}}" />
                        @endif
                    </div>
                    ...
                    <div class="card mt-3">
                        <div class="card-body">
                            <ul class="timeline">
                                @foreach ($leaveApprovalLogs as $item)
                                    @php
