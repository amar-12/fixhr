    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            datatable({
                ...
                url: "{{ route('travel.approval.list') }}",
                ...
                $.ajax({
                   ...
                   url: "{{ route('check.pending.approval.requests') }}",
                    ...
                    success: function(response) {
                        $("#actionDropdownBtn-" + moduleId).html('<i class="fa fa-ellipsis-v"></i>');
                        if(response.status) {
                            if(response.count > 0) {
                                Swal.fire({
                                    icon: 'warning',
                                ...
                                Swal.fire({
                                    ...
                                    if (result.isConfirmed) {
                                        ...
                                        Swal.fire({
                                            ...
                                            preConfirm: (value) => {
                                                if (value !== 'i confirm') {
                                                    Swal.showValidationMessage('You must type "i confirm" to proceed');
                                                }
                                                return value;
                                            },
                                            didOpen: (popup) => {
                                                ...
                                                try {
                                                    ...
                                                    input.name = 'swal-confirm-' + Math.random().toString(36).slice(2);
                                            ...
                                            if (inputResult.isConfirmed && inputResult.value === 'i confirm') {
                                                $.ajax({
                                                   type: "POST",
                            ...
                            Swal.fire({
                                position: 'top-end',
                                icon: 'error',
                                title: 'An error occurred while checking pending requests. ' + response.error,
                                toast: true,
                                showConfirmButton: false,
                                timer: 3000,
                                timerProgressBar: true,
...
@section('content')
    ...
    <div class="modal fade" id="uploadBulkApprovalModal" tabindex="-1" role="dialog" aria-labelledby="uploadBulkApprovalModal" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <form action="{{ route('approval.mapping.import') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="d-flex gap-2">
                            <a href="{{ route('approval.mapping.sample') }}" target="_blank" class="btn btn-outline-primary">
    ...
    <div>
                ...
                <div class="row">
                    <div class="col-md-4">
                        <ol class="breadcrumb breadcrumb-arrow m-0 p-0" style="background: none;">
                            ...
                            <li class="active"><span><b>Approval Flow</b></span></li>
                        </ol>
                    </div>
                        ...
                        <div class="page-rightheader ms-md-auto">
                            <div class="d-flex align-items-end flex-wrap my-auto end-content breadcrumb-end">
                                <div class="d-lg-flex d-block ms-auto">
                                    <div class="btn-list">
                                        @if ($permission->check_route_permission('admin/settings/tada-settings/approval-setting/{id?}', 116))
                                        <a class="btn btn-outline-primary my-auto"
                                            href="{{ url('admin/settings/tada-settings/approval-setting') }}">Add
                                            New Approval</a>
                                        @endif
                                    </div>
                                </div>
        ...
        <div class="row mt-5">
                    ...
                    <div class="card-body">
                         ...
                         <div class="mb-4 d-flex gap-2 align-items-end flex-wrap">
                            ...
                            {{-- @if (session()->has('import_errors_blade'))
                                <a href="{{ route('employee.downloadErrorFile') }}" onclick="location.reload()" class="btn btn-danger">
                                    <i class="fa fa-exclamation-triangle me-2"></i>Download Error File
                                </a>
                            @endif --}}
                        </div>
                        @if (session()->has('import_errors_blade'))
                            <div class="alert alert-warning d-flex align-items-center justify-content-between" role="alert">
                                <div class="d-flex align-items-center">
                                    ...
                                    <span>There were errors in the import. You can download the error file from the link below:</span>
                                </div>
                                <a href="{{ route('employee.downloadErrorFile') }}" onclick="location.reload()" class="btn btn-danger btn-sm ms-3">Download Error File</a>
                            </div>
                        @endif
                        @csrf
                         <div class="row">
         ...
         <div class="col-sm-1" style="padding-left: 1px;padding-right: 1px;height: 10px;margin-top: 31px;margin-left: 25px;">
                                <div class="form-group dropdown">
                                    ...
                                    <ul class="dropdown-menu dropdown-menu-import" aria-labelledby="uploadBulkDropdown">
                                        <li>
                                            <button class="dropdown-item" type="button" data-bs-toggle="modal" data-bs-target="#uploadBulkApprovalModal">
                        ...
                        <div class="table-responsive">
                            <table class="table display table-hover table-vcenter text-wrap border-bottom" id="approval-table-dynamic">
                                <thead>
                                    <tr>
