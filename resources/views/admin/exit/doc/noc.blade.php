...
<body>
    ...
    <div class="letter-container">
        ...
        <div class="text-center mb-4" style="margin-top:80px !important;">
            <h4 class="letter-title">NO OBJECTION CERTIFICATE (NOC)</h4>
        </div>

        <!-- Letter Body -->
        <div>
            <strong>{{ $exit->employee->fh_employee_title->m_name }} {{ $exit->employee->emp_full_name }},</strong><br>

            <p class="text-justify">
                This is to certify that <strong>{{ $exit->employee->emp_full_name }}</strong>
                (Employee Code: {{ $exit->employee->emp_code }})
                was employed with <strong>{{ $exit->employee->fh_business->b_name }}</strong>
                from <strong>{{ \Carbon\Carbon::parse($exit->employee->emp_date_of_joining)->format('d F Y') }}</strong>
                to <strong>{{ \Carbon\Carbon::parse($exit->er_last_working_day)->format('d F Y') }}</strong>.
            </p>

            <p class="text-justify">
                We hereby confirm that the above-mentioned employee has no pending obligations or dues with
                <strong>{{ $exit->employee->fh_business->b_name }}</strong> and we have no objection to
                {{ $exit->employee->emp_full_name }} seeking employment or pursuing further engagements elsewhere.
            </p>
