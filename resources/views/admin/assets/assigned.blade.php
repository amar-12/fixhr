...
@section('content')
        ...
        <div class="row">
            <div class="col-md-4">
                <ol class="breadcrumb breadcrumb-arrow m-0 p-0" style="background: none;">
                    ...
                    <li class="active"><span><b>Assigned Assets</b></span></li>
                </ol>
            </div>
        </div>
    </div>
                        ...
                        <div class="row ">
                            <div id="filterContainer" style="display: none; margin-bottom: 21px;">
                                ...
                                <div class="col-md-2">
                                    ...
                                    <select id="employeeFilter" name="emp_id" data-filter
    ...
    <div class="modal fade" id="unassignSingleModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <form id="unassignSingleForm" method="POST" action="">
                    @csrf
                    @method('PATCH')
                    <div class="modal-body">
                        <div class="alert alert-warning">
                            <strong>Asset:</strong>
                            <span id="singleUnassignAssetTagName"></span> -
                            <span id="singleUnassignAssetTag"></span>
                        </div>
                        {{-- Remark Field --}}
                        <div class="mb-3">
                            <label for="unassign_remark" class="form-label">Remark</label>
                            <textarea name="remark" id="unassign_remark" class="form-control" rows="2"
                                placeholder="Enter reason for unassign..."></textarea>
                        </div>
                    </div>
    ...
    <div class="modal fade" id="replaceModal" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">Replace Asset</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                </div>
                <form id="replaceForm" method="POST">
                    ...
                    <input type="hidden" name="asset_tag" id="hiddenAssetTag">
                    <div class="modal-body">

                        <div class="alert alert-warning">
                            <strong>Asset:</strong>
                            <span id="originalAssetTagName"></span> -
                            <span id="originalAssetTag"></span>
                        </div>

                        <div class="mb-3">
                            <label class="form-label">Replacement Asset</label>
                            <select name="replacement_asset_id" class="form-select" required>
                                <option value="">Choose a replacement...</option>
                                @foreach (\App\Models\Asset::stock()->with('assetType')->get() as $stockAssets)
                                    <option value="{{ $stockAssets->id }}">
                                        {{ $stockAssets->asset_tag }} - {{ $stockAssets->assetType->name }}
                                        ({{ $stockAssets->model_number }})
                                    </option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancel</button>
                        <button type="submit" class="btn btn-success">Replace Asset</button>
                    </div>
                </form>
            </div>
        </div>
        ...
        <div class="modal-dialog modal-lg"> <!-- Large modal for enough width -->
            <div class="modal-content">
                ...
                <form id="serviceForm" method="POST" enctype="multipart/form-data">
                    ...
                    <input type="hidden" id="serviceAssestempid" name="serviceAssestempid">
...
@section('script')
    ...
    <script>
        $('#replaceModal').on('show.bs.modal', function(event) {
            ...
            const modal = $(this);
            modal.find('#originalAssetTag').text(assetTag);
            modal.find('#originalAssetTagName').text(assetsName);
            modal.find('#replaceForm').attr('action', "{{ url('assets') }}/" + assetId + "/replace");


            modal.find('#hiddenAssetId').val(assetId);
            modal.find('#hiddenAssetTag').val(assetTag);
        });
    </script>
    ...
    <script>
        // Handle single asset unassignment
        $(document).on('click', '.unassign-single-btn', function() {
            const assetId = $(this).data('asset-id');
            const assetTag = $(this).data('asset-tag');
            const assetsName = $(this).data('asset-name'); // <-- fixed

            $('#singleUnassignAssetTag').text(assetTag);
            $('#singleUnassignAssetTagName').text(assetsName);

            const formAction = `/assets/${assetId}/unassign`;
            $('#unassignSingleForm').attr('action', formAction);

            $('#unassignSingleModal').modal('show');
        });
        ...
        $('#serviceModal').on('show.bs.modal', function(event) {
            ...
            const assetTag = button.data('asset-tag');
            const assetsName = button.data('asset-name');
            const employeeID = button.data('asset-empid');

            const modal = $(this);
            modal.find('#serviceAssetTag').text(assetTag);
            modal.find('#serviceAssetName').text(assetsName);
            modal.find('#serviceAssestempid').val(employeeID);
