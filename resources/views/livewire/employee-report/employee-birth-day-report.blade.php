    ...
    <div class="col-md-10">
        ...
        <div class="ef-panel">
            ...
            <div class="ef-panel-header">
                <span class="ef-panel-title">
                    {{ $slug === 'employee-birthday' ? 'Employee Birthday Report Filters' : 'Employee Joining Report Filters' }}
                </span>
                {{-- Filter toggle --}}
                        ...
                        @foreach ($filterFields as $key => $label)
                            @if (array_key_exists($key, $filters))
                                <div class="ef-switch-row">
                                    <label for="{{ $key }}BdSwCb">{{ $label }}</label>
            ...
            <div class="row gx-3 gy-3 ef-fields-grid {{ $showFilterPanel ? 'ef-grid-shrink' : '' }}">
                ...
                <div class="col-md-6">
                    ...
                    <div class="ef-input-wrap">
                        <input type="date" wire:model.live="selectedFromDate" class="ef-control">
                        <span class="ef-icon">
                ...
                <div class="col-md-6">
                    ...
                    <div class="ef-input-wrap">
                        <input type="date" wire:model.live="selectedToDate" class="ef-control">
                        <span class="ef-icon">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" stroke="currentColor"
                                stroke-width="2" viewBox="0 0 24 24">
                                <rect x="3" y="4" width="18" height="18" rx="2" ry="2" />
                ...
                @if ($filters['designation'] ?? false)
                    <x-search-input label="Designation" wireModel="searchDesignation" :results="$designations"
                        selectMethod="selectDesignation" idField="dg_id" nameField="dg_name" :selected="$selectedDesignationId ? $searchDesignation : null"
                ...
                @if ($filters['dealership'] ?? false)
                    <x-search-input label="Dealership" wireModel="searchDealer" :results="$dealers"
                        selectMethod="selectDealer" idField="dlr_id" nameField="dlr_name" :selected="$selectedDealerId ? $searchDealer : null"
