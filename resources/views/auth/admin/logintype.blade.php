    ...
    <body id="my-view">
        ...
        <div class="card col-md-12 col-lg-12 p-5">
            <div class="row align-items-center ">
                ...
                <?php
                $Helper = new App\Helpers\CentralLogicss();
            ...
            @php
                $guestUser = DB::table('guest_user')->where('guest_email', Session::get('email'))->first();
                $guestBusiness = DB::table('business_details_list')
                    ->where('business_id', $guestUser->business_id ?? '')
                    ->first();
            @endphp

            <div class="col-md-12 col-sm-6 col-lg-12 owner-card p-0" id="{{ $guestUser->guest_id ?? 'default-guest' }}">
                <form action="{{ route('admin.handleCard') }}" method="post" id="guestUserForm">
                    @csrf
                    @if ($guestUser)
                        <div class="offer offer-radius offer-primary" onclick="submitForm()">
                            <div class="shape">
                                <div class="shape-text">User</div>
                                <div class="shape-id" hidden>{{ $guestBusiness->business_id }}</div>
                            </div>
                            <input type="text" name="business_id" value="{{ $guestBusiness->business_id }}" hidden>
                            <input type="text" name="guest_email" value="{{ $guestUser->guest_email }}" hidden>
                            <input type="text" name="login_type" value="3" hidden>
                            <div class="offer-content">
                                <h3 class="lead font-weight-semibold"><b>{{ $guestBusiness->business_name }}</b></h3>
                                <p>To Record Attendance Details of My Employees</p>
                            </div>
                        </div>
                    @endif
                </form>
            ...
            <script>
                function submitForm() {
                    document.getElementById('guestUserForm').submit();
                }
            </script>
