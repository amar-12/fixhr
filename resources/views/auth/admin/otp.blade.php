        ...
        <div class="row align-items-center">
            <div class="col-md-12">
                <div class="card-body">
                    ...
                    <span>OTP Sent To <span class="text-primary" id="maskedEmail"></span></span>

                    <script>
                        // Assuming Session::get('firstEmail') returns the email address
                        var email = "{{ Session::get('firstEmail') }}";

                        // Function to mask email
                        function maskEmail(email) {
                            var atIndex = email.indexOf('@');
                            var maskedEmail = email.substring(0, 3) + '********' + email.substring(atIndex);
                            return maskedEmail;
                        }

                        // Set masked email to the span
                    ...
                    <form method="POST" action="{{ route('businessVerify') }}">
                        ...
                        <div class="input-group  mt-3 mb-2">
                            <input type="text" name="otp" id="otp" class="form-control"
                                placeholder="Enter Your 6 Digit OTP" minlength="6" maxlength="6"
                                onkeypress="return numericOnly(event)" required autofocus>
                        </div>
                        @if ($errors->has('otpError'))
                            <span style="color: red" id="otpError">
                                {{ $errors->first('otpError') }}
                            </span>
                        @endif
