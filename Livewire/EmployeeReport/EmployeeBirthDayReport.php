...
class EmployeeBirthDayReport extends Component
{
    ...
    public $selectedEmployeeId=null, $employeeStatusId=null, $selectedDepartmentId=null, $selectedDesignationId=null, $selectedDealerId=null;
    ...
    public function generateReport()
    {
        ...
        if ($this->slug == 'employee-birthday') {
            $from = Carbon::parse($this->selectedFromDate)->format('m-d'); // e.g. 01-01
            $to   = Carbon::parse($this->selectedToDate)->format('m-d');   // e.g. 12-31
        ...
        if ($this->slug == 'employee-joining') {
            ...
            $query = Employee::with(['fh_department', 'fh_designation', 'fh_dealership'])
                ...
                ->when($this->selectedEmployeeId, fn($q, $v) => $q->where('emp_id', $v))
                ->when($this->employeeStatusId, fn($q, $v) => $q->where('emp_status', $v))
                ->orderBy('emp_date_of_joining', 'ASC');
        }
