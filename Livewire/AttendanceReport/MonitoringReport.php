...
class MonitoringReport extends Component
{
    ...
    public function generateReport()
    {
        ...
        // Dispatch alert to inform user that report generation has started
        $this->dispatch('show-alert', [
            'type' => 'info', // Using 'info' to indicate processing, not completion
            'message' => 'Report generation started. This may take some time depending on the data size. Please wait...',
        ]);
        set_time_limit(0); // Remove execution time limit for this request
    ...
    'emp_code',
        ...
        foreach ($employees as $employee) {
            ...
            foreach ($attendanceData as $data) {
                ...
                // Apply attendance status filters based on slug
                ...
                // Apply dealer filter (equivalent to whereHas('fh_employees_details'))
                $matchesDealer = !$this->selectedDealerId || ($employee->emp_dlr_id == $this->selectedDealerId);
                if ($matchesShift && $matchesWorkMode && $matchesAttendanceStatus && $matchesDealer) {
                    $records->push($record);
                }
            }
        }
